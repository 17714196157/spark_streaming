...
import numpy as np
import pandas as pd
from code import mongodb_Data_cleaning_to_csv
from code import Industry_keyword_interactive
...
def modify_line(iline):
    ...
    :return:
    """
    # column_name_list = column_name_list_broadcast.value
...
def out_to_kafka(iter):
    ...
    with topic.get_sync_producer() as producer:
        ...
        for record in iter:
            ...
            # json 化数据 插入mongdb
            # data_pd = pd.DataFrame([record_list], columns=column_name_list)
            # data_json = data_pd.to_json('records')
            data_json = dict(zip(column_name_list, record_list))
            print("@@@@@@@@@@@@@@@@@", record_list, data_json)
